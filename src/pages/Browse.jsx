...
export default function Browse() {
  ...
  const filteredMentors = selectedSkill
    ? mentors.filter((m) => Array.isArray(m.skills) && m.skills.includes(selectedSkill))
    : mentors;
  ...
  return (
    <div className="min-h-screen font-sans bg-[#0B1C2D] relative overflow-x-hidden selection:bg-[#9B4D5E] selection:text-white flex flex-col">
      <DashboardHeader title="Browse Mentors" />
      ...
      <div className="relative z-10 px-6 py-12">
        <h1 className="text-4xl font-bold mb-10 text-center text-[#9B4D5E]">Browse Mentors</h1>
        <div className="mb-8 flex flex-wrap justify-center gap-4">
          <button
            className={`px-4 py-2 rounded-full border border-[#9B4D5E] text-white bg-transparent hover:bg-[#9B4D5E] transition ${selectedSkill === "" ? "bg-[#9B4D5E]" : ""}`}
          ...
          {skills.map((skill) => (
            <button
              ...
              className={`px-4 py-2 rounded-full border border-[#9B4D5E] text-white bg-transparent hover:bg-[#9B4D5E] transition ${selectedSkill === skill.name ? "bg-[#9B4D5E]" : ""}`}
        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {filteredMentors.length === 0 ? (
              <div className="text-slate-400 col-span-3 text-center">No mentors found for selected skill.</div>
